...
/**
 * Class implementing a split on an attribute.
 */
public class GiniSplitModel
    ...
    /** Gini score of split.  */
    private double m_giniScore;

    /** The sum of the weights of the instances. */
    private double m_sumOfWeights;

    /** Static reference to splitting criterion. */
    private static GiniSplitCriterion m_giniSplitCrit = new GiniSplitCriterion();

    /**
     * Initializes the split model.
     */
    public GiniSplitModel(int attIndex, int minNoObj, double sumOfWeights,
        ...
        for (i = 0; i < numAttValues; i++){
            ...
            if (Utils.grOrEq(newDistribution.perBag(i),m_minNoObj)){
                ...
                if (secondDistribution.check(m_minNoObj)){
                    m_numSubsets = 2;
                    currGini = m_giniSplitCrit.splitCritValue(secondDistribution);
//                    currGini = m_gainRatioCrit.splitCritValue(secondDistribution,
//                            m_sumOfWeights);
                    if ((i == 0) || Utils.gr(currGini, m_giniScore)){
                        m_giniScore = currGini;
//                        m_infoGain = currIG;
    ...
    private void handleNumericAttribute(Instances trainInstances)
        ...
        while (next < firstMiss){
            ...
            if (trainInstances.instance(next-1).value(m_attIndex)+1e-5 <
                ...
                if (Utils.grOrEq(m_distribution.perBag(0),minSplit) &&
                    ...
                    currentGini = m_giniSplitCrit.
                            splitCritValue(m_distribution);
                    if (Utils.gr(currentGini,m_giniScore)){
                        m_giniScore = currentGini;
                        splitIndex = next-1;
                    }
                    index++;
                }
        ...
        if (m_useMDLcorrection) {
            m_giniScore = m_giniScore-(Utils.log2(index)/m_sumOfWeights);
        }
        if (Utils.smOrEq(m_giniScore,0))
            return;
        ...
        // Compute modified gini for best split.
        m_giniScore = m_giniSplitCrit.
                splitCritValue(m_distribution);
    }
