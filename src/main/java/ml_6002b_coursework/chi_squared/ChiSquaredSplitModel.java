 ...
 */
public class ChiSquaredSplitModel
    ...
    /** GainRatio of split.  */
    private double m_chiSq;

    /** The sum of the weights of the instances. */
    private double m_sumOfWeights;

    /** Static reference to splitting criterion. */
    private static ChiSquaredSplitCriterion m_chiSqCrit = new ChiSquaredSplitCriterion();
...
//  private static GainRatioSplitCrit m_gainRatioCrit = new GainRatioSplitCrit();
     ...
     */
    public ChiSquaredSplitModel(int attIndex, int minNoObj, double sumOfWeights,
        ...
        for (i = 0; i < numAttValues; i++){
            ...
            if (Utils.grOrEq(newDistribution.perBag(i),m_minNoObj)){
                ...
                if (secondDistribution.check(m_minNoObj)){
                    m_numSubsets = 2;
                    currGR = m_chiSqCrit.splitCritValue(secondDistribution);
                    if ((i == 0) || Utils.gr(currGR, m_chiSq)){
                        m_chiSq = currGR;
    ...
    private void handleNumericAttribute(Instances trainInstances)
        ...
        // indices.
//    defaultEnt = m_chiSqCrit.oldEnt(m_distribution);
        while (next < firstMiss){
            ...
            if (trainInstances.instance(next-1).value(m_attIndex)+1e-5 <
                ...
                if (Utils.grOrEq(m_distribution.perBag(0),minSplit) &&
                    ...
                    currentChiSq = m_chiSqCrit.splitCritValue(m_distribution);
                    if (Utils.gr(currentChiSq,m_chiSq)){
                        m_chiSq = currentChiSq;
                        splitIndex = next-1;
                    }
                    index++;
                }
        ...
        // Compute modified chi squared for best split.
        if (m_useMDLcorrection) {
            m_chiSq = m_chiSq-(Utils.log2(index)/m_sumOfWeights);
        }
        if (Utils.smOrEq(m_chiSq,0))
            return;
        ...
        m_chiSq = m_chiSqCrit.splitCritValue(m_distribution);
