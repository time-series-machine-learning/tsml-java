...
//        classifierNames[7] = "bayesNet";
     ...
     * @param classifiers the names of classifiers that can all be found in ClassificationExperiments.setClassifier(...)
    ...
    protected static void buildCAWPEPaper_BuildClassifierResultsFiles(String baseWritePath, String[] dataHeaders, String[] dataPaths,
        ...
        for (int archive = 0; archive < dataHeaders.length; archive++) {
            for (String classifier : classifiers) {
                if (!ClassificationExperiments.beQuiet)
                ...
                for (String dset : datasetNames[archive]) {
                    ...
                    for (int fold = 0; fold < numFolds; fold++) {
                        ...
                        Optional:
                            ...
                            */
                        ClassificationExperiments.main(new String[] { "-dp="+dataPaths[archive], "-rp="+baseWritePath+dataHeaders[archive]+"/", "-cn="+classifier, "-dn="+dset, "-f="+(fold+1), "-gtf=true"});
                    }
                }
            }
        }
...
//                        exp.performTimingBenchmark = true;
                        ClassificationExperiments.runExperiment(exp,data[0],data[1],c);
                    }
                }
            }
        }
