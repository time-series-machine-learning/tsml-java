...
public class ForestExperiments{
    public static String[] classifiers={"TunedRandF","TunedRotF","RotF10","RotF20","RotF30","RotF40","RotF50","RotF60","RotF70","RotF80","RotF90","RotF100","RotF200","RotF300","RotF400","RotF500","RandF10","RandF20","RandF30","RandF40","RandF50","RandF60","RandF70","RandF80","RandF90","RandF100","RandF200","RandF300","RandF400","RandF500","RandF600","RandF700","RandF800","RandF900","RandF1000","RandF1600","RandF2000"};
...
//Generates txt files to run jobs for a single classifier        
        ...
        for(String s:classifiers){
            ...
            for(String a:fileNames){
                ...
                of.writeLine("java -jar Forest.jar "+s+" "+a+" $LSB_JOBINDEX");                
                if(grace)
...
//            c.setNumFeaturesRange(new double[]{3});
           ...
           return c; 
            ...
            String clsType=classifier.substring(0, 5);
...
// just testing whether tuning the number of trees helps
    //            RandomForest c=new RandomForest();
                int nTrees=Integer.parseInt(classifier.substring(5));
                c.setNumTrees(nTrees);
                System.out.println(" Rand Forest with "+nTrees+" trees");
                return c;
            }
            else{ //RotF{
                RotationForest c=new RotationForest();
                int nTrees=Integer.parseInt(classifier.substring(4));
                c.setNumIterations(nTrees);
                System.out.println(" Rotation Forest with "+nTrees+" trees");
                return c;
            }
        }
    }
 ...
 */       if(args.length>0){//Cluster run
            ...
            String[] paras={"TunedRandF","monks-1","6"};
