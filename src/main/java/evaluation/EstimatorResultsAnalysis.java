 ...
 */
public class EstimatorResultsAnalysis {
...
//    public static PerformanceMetric benchmarkedEstimateTimeMetric = PerformanceMetric.fromScratchEstimateTimeBenchmarked;
    ...
    public static class EstimatorEvaluation {
        public String estimatorName;
        public EstimatorResults[][] testResults; //[dataset][fold]
        public EstimatorResults[][] trainResults; //[dataset][fold]

        public EstimatorEvaluation(String name, EstimatorResults[][] testResults, EstimatorResults[][] trainResults) {
     ...
     * @param metrics a list of PerformanceMetrics that effectively are able to summarise a EstimatorResults
    ...
    public static void performFullEvaluation(
            ...
            EstimatorResultsCollection results,
            Map<String, Map<String, String[]>> dsetGroupings)
    {
        //hacky housekeeping
        MultipleEstimatorsPairwiseTest.beQuiet = true;
...
//                String[] groupSummaryFileStrings = eval_metricOnSplit(groupPath+statName+"/", groupFileName, groupName, evalSet, statName, groupFoldVals, cnames, groupDsets, null);
    ...
    protected static String[] eval_metric(String outPath, String filename, EstimatorResultsCollection results, PerformanceMetric metric, Map<String, Map<String, String[]>> dsetGroupings) throws Exception {
        String statName = metric.name;
        outPath += statName + "/";
        new File(outPath).mkdirs();

        String[] cnames = results.getEstimatorNamesInOutput();
    ...
    protected static String[/*{train,test}*/][] eval_CompResourcesRAW(String outPath, String filename, EstimatorResultsCollection results, Map<String, Map<String, String[]>> dsetGroupings) throws Exception {
        String[] cnames = results.getEstimatorNamesInOutput();
        String[] dsets = results.getDatasetNamesInOutput();

        computationalDiaFolderName = computationalDiaFolderName_raw;

        PerformanceMetric trainTimeMetric = EstimatorResultsAnalysis.trainTimeMetric;
...
//        return new String[][] { trainResStr, testResStr, estimateResStr };
    ...
    protected static String[/*{train,test}*/][] eval_CompResourcesBENCHMARKED(String outPath, String filename, EstimatorResultsCollection results, Map<String, Map<String, String[]>> dsetGroupings) throws Exception {
        String[] cnames = results.getEstimatorNamesInOutput();
        String[] dsets = results.getDatasetNamesInOutput();
...
//                    if (orderedAccs[rank] == new BigDecimal(accs[c][d], new MathContext(DEFAULT_MATLAB_PRECISION, RoundingMode.DOWN)).doubleValue()) {
     ...
     */
    private static double[][][] getTimingsIfAllArePresent(List<EstimatorEvaluation> res, Function<EstimatorResults, Double> getter) {
    ...
    protected static double[][][] getInfo(List<EstimatorEvaluation> res, Function<EstimatorResults, Double> getter, String trainortest) {
        double[][][] info = new double[res.size()][res.get(0).testResults.length][res.get(0).testResults[0].length];
        for (int i = 0; i < res.size(); i++) {
    ...
    protected static String[] getNames(List<EstimatorEvaluation> res) {
        String[] names = new String[res.size()];
        for (int i = 0; i < res.size(); i++)
            names[i] = res.get(i).estimatorName;
        return names;
    }
