...
public class ExperimentalArguments implements Runnable {
    ...
    @Parameter(names = {"-dp", "--dataPath"}, required = true, order = 0, description = "(String) The directory that contains the dataset to be evaluated on, in the form "
            + "[--dataPath]/[--datasetName]/[--datasetname].arff (the actual arff file(s) may be in different forms, see ClassificationExperiments.sampleDataset(...).")
    ...
    @Parameter(names = {"-dn", "--datasetName"}, required = true, order = 3, description = "(String) The name of the dataset to be evaluated on, which resides within the dataPath in the form "
            + "[--dataPath]/[--datasetName]/[--datasetname].arff (the actual arff file(s) may be of different forms, see ClassificationExperiments.sampleDataset(...).")
    ...
    public void run() {
        try {
            ClassificationExperiments.setupAndRunExperiment(this);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
     ...
     */
    public List<ExperimentalArguments> generateClassifierExperiments(String[] classifierNames, List<Supplier<Classifier>> classifierGenerators, String[] datasetNames, int minFold, int maxFold) {
    ...
    private void parseArguments(String[] args) throws Exception {
        ...
        jc.setProgramName("ClassificationExperiments.java");  //todo maybe add copyright etcetc
        try {
            jc.parse(args);
        } catch (Exception e) {
            if (!help) {
...
//                System.exit(1);
        ...
        ClassificationExperiments.debug = this.debug;
