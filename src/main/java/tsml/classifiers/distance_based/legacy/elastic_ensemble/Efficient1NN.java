 ...
 */
package tsml.classifiers.distance_based.legacy.elastic_ensemble;
...
import tsml.classifiers.distance_based.legacy.elastic_ensemble.fast_elastic_ensemble.CandidateNN;
import tsml.classifiers.distance_based.legacy.elastic_ensemble.fast_elastic_ensemble.utils.SequenceStatsCache;
...
public abstract class Efficient1NN extends EnhancedAbstractClassifier implements SaveParameterInfo{
    ...
    public abstract double lowerBound(final Instance query, final Instance candidate, final int queryIndex, final int candidateIndex);

    public abstract double lowerBound(final Instance query, final Instance candidate, final int queryIndex, final int candidateIndex, final double cutOffValue);

    public abstract double lowerBound(final Instance query, final Instance candidate, final int queryIndex, final int candidateIndex, final SequenceStatsCache cache);

    public abstract double lowerBound(final Instance query, final Instance candidate, final int queryIndex, final int candidateIndex, final double cutOffValue, final SequenceStatsCache cache);
...
//    }
     ...
     ************************************************************************************************/
    CandidateNN[][] candidateNNS; // a table to store nearest neighbours for each instance in the training set
    final int nParams = 100; // number of parameters to learn
    int[][][] classCounts;
    int maxWindow;
    SequenceStatsCache trainCache; // cache to store some pre-computed data on the training set
    ...
    public abstract void initNNSTable(final Instances trainData, final SequenceStatsCache cache);

    public abstract void initApproxNNSTable(final Instances trainData, final SequenceStatsCache cache, final int nSamples);

    public double[] fastParameterSearch(final Instances train) throws Exception {
        double[] accAndPreds;
        this.maxWindow = train.numAttributes() - 1;
    ...
    private double[] fastParameterAccAndPred(final Instances train, final int paramId, final int n) {
        this.setParamsFromParamId(train, paramId);
        int correct = 0;
        double pred, actual;
    ...
    public double[] approxParameterSearch(final Instances train, final int nSamples) throws Exception {
        final long start = System.nanoTime();
        double[] accAndPreds;
        this.maxWindow = train.numAttributes() - 1;
        this.trainCache = new SequenceStatsCache(train, train.numAttributes() - 1);

        initApproxNNSTable(train, trainCache, nSamples);
        ...
        if (fileWriting) {
            String parsedFileName = this.outputDir + this.classifierIdentifier + "/ApproxEE_Predictions/" +
                    datasetName + "/trainFold" + resampleId + ".csv";
    ...
    private double[] approxParameterAccAndPred(final Instances train, final int paramId, final int nSamples) {
        this.setParamsFromParamId(train, paramId);
        int correct = 0;
        double pred, actual;

        final double[] accAndPreds = new double[nSamples + 1];
        for (int i = 0; i < nSamples; i++) {
            actual = train.get(i).classValue();
            pred = train.get(candidateNNS[paramId][i].index).classValue();
            if (pred == actual) {
                correct++;
            }
            accAndPreds[i + 1] = pred;
        }
        accAndPreds[0] = 1.0 * correct / nSamples;
