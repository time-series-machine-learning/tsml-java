...
public class RISE_RISEProxy extends EnhancedAbstractClassifier implements TrainTimeContractable, TechnicalInformationHandler, Checkpointable, Tuneable {
...
/**** Checkpointing variables *****/
     ...
     * @param seed
     */
    public RISE_RISEProxy(long seed){
 ...
 identifier.
    ...
    private RISE_RISEProxy readSerialise(long seed){
        ObjectInputStream oi = null;
        RISE_RISEProxy temp = null;
        try {
            ...
            temp = (RISE_RISEProxy)oi.readObject();
            oi.close();
            fi.close();
            System.out.println("File load successful: " + ((RISE_RISEProxy)temp).classifiersBuilt + " trees.");
    ...
    public void copyFromSerObject(Object temp){
        RISE_RISEProxy riseKNNProxy;
        try{
            riseKNNProxy =(RISE_RISEProxy)temp;
        }catch(Exception ex){
            throw new RuntimeException(" Trying to load from ser object thar is not a RISE object. QUITING at copyFromSerObject");
        }

        this.baseClassifiers = riseKNNProxy.baseClassifiers;
        this.classifier = riseKNNProxy.classifier;
        this.data = riseKNNProxy.data;
        this.downSample = riseKNNProxy.downSample;
        this.PS = riseKNNProxy.PS;
        this.intervalsAttIndexes = riseKNNProxy.intervalsAttIndexes;
        this.intervalsInfo = riseKNNProxy.intervalsInfo;
        this.maxIntervalLength = riseKNNProxy.maxIntervalLength;
        this.minIntervalLength = riseKNNProxy.minIntervalLength;
        this.numClassifiers = riseKNNProxy.numClassifiers;
        this.rand = riseKNNProxy.rand;
        this.rawIntervalIndexes = riseKNNProxy.rawIntervalIndexes;
        this.checkpointPath = riseKNNProxy.checkpointPath;
        this.stabilise = riseKNNProxy.stabilise;
        this.timer = riseKNNProxy.timer;
        this.transformType = riseKNNProxy.transformType;
        this.classifiersBuilt = riseKNNProxy.classifiersBuilt;
        this.startEndPoints = riseKNNProxy.startEndPoints;
...
//                        checkpoint(startTime);
    ...
    private void estimateOwnPerformance(Instances data) throws Exception{
        ...
        if(estimator==EstimatorMethod.OOB) {
            ...
            RISE_RISEProxy riseKNNProxy = new RISE_RISEProxy();
//NEED TO SET PARAMETERS
//            rise.copyParameters(this);
            if (seedClassifier)
                riseKNNProxy.setSeed(seed * 100);
            if (trainTimeContract) {//Set the contract for each fold
                riseKNNProxy.setTrainTimeLimit((long)(((double) timer.forestTimeLimit * perForBag) / (numFolds - 2)));
            }
            riseKNNProxy.setEstimateOwnPerformance(false);
            trainResults = cv.evaluate(riseKNNProxy, data);
            long est2 = System.nanoTime();
 ...
 Dividing this by minNumtrees - treeCount gives the maximum space such that in the worse case the contract is met.
    ...
    public static void main(String[] args) {
        ...
        final String PROBLEM = "CatsDogs";
        final String EXTENSION = "arff";
        Instances data = loadDataNullable(PATH + "\\" + PROBLEM + "\\" + PROBLEM + "." + EXTENSION);
        ...
        RISE_RISEProxy RISE_KNNProxy = null;
        ...
        System.out.println("\n");
        try {
            RISE_KNNProxy = new RISE_RISEProxy();
