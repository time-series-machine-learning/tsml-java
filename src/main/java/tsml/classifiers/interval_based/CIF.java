...
public class CIF extends EnhancedAbstractClassifier
        implements TechnicalInformationHandler, TrainTimeContractable, Checkpointable, Tuneable {
    ...
    private EstimatorMethod estimator = EstimatorMethod.OOB;
        ...
        while((System.nanoTime()-trainResults.getBuildTime()- checkpointTimeDiff < contractTime
            ...
            for (int j = 0; j < numIntervals; j++) {
                if (rand.nextBoolean()) {
                    ...
                    int range = interval[j][1] > maxIntervalLength
                            ? maxIntervalLength : interval[j][1];
                    int length;
